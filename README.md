...
<body>
    ...
    <div id="checkout-form">
        ...
        <select id="delivery-method" class="input-field" onchange="updateTotal()">
            <option value="0">Boutique Pickup (₱0)</option>
            <option value="50">Local Delivery (₱50)</option>
            <option value="100">Extended Area (₱100)</option>
        </select>
    </div>
    ...
    <script>
        ...
        function renderProducts() {
            ...
            list.innerHTML = filtered.map(p => `
                <div class="product-card">
                    ...
                    <div class="card-header"><span>${p.name}</span><span>₱${p.price}/kg</span></div>
        ...
        function addWithNote(id) {
            ...
            cart.push({ name: p.name, desc: "₱" + budgetVal, price: parseFloat(budgetVal), note: note });
            saveCartState(); feedback(id);
        }

        function feedback(id) {
            const btn = document.getElementById(`btn-${id}`);
            btn.innerText = "✓ Added"; btn.classList.add('success');
        ...
        function updateTotal() {
            ...
            cartDiv.innerHTML = cart.length ? cart.map((item, index) => `
                <div class="cart-item">
                    <div class="cart-row"><span>${item.name} (${item.desc})</span><span>₱${item.price.toFixed(2)} <button class="remove-btn" onclick="removeItem(${index})">&times;</button></span></div>
                </div>
        ...
        async function handleOrder(e) {
            ...
            let orderSummary = "✨ RHODA'S MARKET ORDER ✨\n\n";
            cart.forEach(i => {
                orderSummary += `• ${i.name} (${i.desc}) - P${i.price.toFixed(2)}\n`;
                if(i.note) orderSummary += `  Note: ${i.note}\n`;
            });
            orderSummary += `\n🏠 Address: ${addr}`;
            orderSummary += `\n💰 Total: P${document.getElementById('grand-total').innerText}`;
